...
- **Allowed deps:** `src/core`, `src/config`.
- **Forbidden:** business logic of any kind. `src/cli` must not talk to
  `src/artifacts` or `src/providers` directly — if the CLI needs
  artifact data, it goes through `src/core`, so a future UI/API consumes
  the exact same path (`docs/artifacts.md` §8).

### `src/core/`
- **Owns:** Pipeline Runner, Skill Registry, Run State, orchestration of
  a skill's execution lifecycle (`docs/skills.md` §6).
- **Allowed deps:** `src/artifacts` (to persist output), `src/providers/gateway`
  (interface only), `src/config`.
- **Forbidden:** importing anything from `src/cli`, `src/providers/adapters`
  (must go through the gateway), or any specific skill's internals. The
  Core Engine knows skills only through the manifest + subprocess
  protocol, never by importing skill code.
...
- **Allowed deps:** `src/config` (storage root path), standard library.
- **Forbidden:** depending on `src/providers` or `src/cli` or any skill —
  artifacts are a data layer, agnostic to what produced them.

### `src/providers/gateway/`
...
- **Allowed deps:** `src/providers/adapters` — but only through a
  registry/factory keyed by config, never a direct import of a specific
  adapter's class from outside that registry.
...
- **Forbidden:** being imported from anywhere except the gateway's
  adapter registry — not from `src/core`, not from a skill, not from
  another adapter.
...
- **Forbidden:** being depended upon by `src/core`, `src/cli`, or any
  skill — the dependency arrow points one direction. Core must be
  buildable and runnable with the entire `plugins/` directory deleted.
...
- **Owns:** no code, but each doc has an implicit owning module (this
  doc ↔ the whole tree; `docs/skills.md` ↔ `src/core` + `skills/`, etc.).
...
- **Owns:** nothing but generated output. Git-ignored. No module may
  ...
  `src/artifacts`'s read interface, never a raw path join, so the
  on-disk layout can change without breaking every caller.
